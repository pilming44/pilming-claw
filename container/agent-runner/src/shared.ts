/**
 * Shared utilities for NanoClaw agent runners (Claude and OpenAI).
 * Output protocol, logging, IPC functions, and conversation archival.
 */
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (data.type === 'message' && data.text) {
          messages.push({ text: data.text, requestId: data.requestId });
        }
      } catch (err) {
        log(
    ...
    const poll = () => {
      ...
      if (messages.length > 0) {
        resolve({
          ...
          requestId: messages[messages.length - 1].requestId,
        });
        return;
      }
